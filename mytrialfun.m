...
hdr   = ft_read_header(cfg.dataset, 'headerformat', 'ctf_old');
...
cfg.trialdef.eventvalueresp.left = 128; % CCW response (other left buttons are probably 16, 32, 64)
cfg.trialdef.eventvalueresp.right = 8; % CW response (other right buttons are probably 1, 2, 4)
...
cfg.trialdef.prestim    = 1.6-lag; % in seconds (0.8s fixation cross and 0.8s cue)
cfg.trialdef.poststim   = cfg.trialdef.eventduration+0.26+lag; % select data until 0.26 seconds after response (0.26s poststimulus for TFA)
